...
// This variant will make cards fade in and slide up slightly more distinctly.
const cardVariants = {
  offscreen: {
  ...
  onscreen: {
    ...
    transition: {
      type: "spring",
      bounce: 0.4,
      duration: 1, // Longer duration for a smoother reveal
      staggerChildren: 0.1 // Stagger internal elements if you add them later
    }
  }
};
...
const AboutSection = () => {
  return (
    <div className='min-h-screen flex flex-col justify-center items-center py-16 px-4 bg-gray-50'> {/* Changed background to slightly off-white */}
       ...
       <div className="flex flex-col md:w-1/2 p-4 md:mt-40 space-y-8 max-h-[85vh] md:max-h-[90vh] overflow-y-auto hide-scrollbar pr-2">{/* Increased max-h, added pr-2 for scrollbar room */}
          {cardData.map((card) => (
            <Motion.div
              ...
              viewport={{ once: false, amount: 0.3 }} // Trigger when 30% of card is visible, can re-trigger
              variants={cardVariants}
              whileHover={{ scale: 1.03, boxShadow: "0px 15px 25px rgba(0, 0, 0, 0.15)" }} // More pronounced hover
              transition={{ type: "spring", stiffness: 300, damping: 20 }} // Add transition to hover
              className='relative cursor-target bg-white p-8 rounded-xl shadow-xl  border border-gray-100' // Better styling
            >
      ...
      {/* Basic Custom Scrollbar Styling (add this to your global CSS, e.g., index.css or a dedicated CSS file) */}
      <style jsx>{`
