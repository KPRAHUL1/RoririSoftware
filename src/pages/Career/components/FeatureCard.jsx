...
// A simple wrapper for animated icons
const AnimatedIcon = ({ children, colorClass }) => (
  <motion.div
    initial={{ scale: 0, opacity: 0 }}
    animate={{ scale: 1, opacity: 1 }}
    transition={{ type: "spring", stiffness: 260, damping: 20 }}
    // The text-color class from Tailwind will now set the currentColor for the SVG
...
const FeatureCard = ({ icon, title, description, animationDelay }) => {
  return (
    ...
    >
      {icon} {/* The icon is now passed as a pre-wrapped AnimatedIcon */}
  ...
  const features = [
    {
      icon: (
        <AnimatedIcon colorClass="text-yellow-500">
          {/* Synchronized Environment: Two interlocking gears */}
    ...
    },
    {
      icon: (
        <AnimatedIcon colorClass="text-green-500">
          {/* Growth Opportunities: Line chart with an upward trend */}
          <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      icon: (
        <AnimatedIcon colorClass="text-pink-500">
          {/* Meaningful Work: Document with a star/checkmark */}
          <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
            <path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/>
            <path d="M12 12.72L9.41 10.13L8 11.54L12 15.54L18 9.54L16.59 8.13L12 12.72z" fill="#fff"/> {/* This path might be the issue, let's remove it or make it transparent for a clear background */}
            {/* Alternative for meaningful work - simpler document and checkmark */}
            <path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/>
    ...
    },
    {
      icon: (
        <AnimatedIcon colorClass="text-blue-500">
          {/* Work-Life Balance: Classic balance scales */}
          <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="bg-white py-16 px-4">
      <div className="container mx-auto">
        <motion.h2
          className="text-3xl md:text-4xl font-extrabold text-gray-800 mb-12 text-center md:text-left"
          initial="hidden"
          animate="visible"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
          {features.map((feature, index) => (
            <FeatureCard
              ...
              animationDelay={0.4 + index * 0.2} // Staggered animation
            />
          ))}
        </div>
      </div>
